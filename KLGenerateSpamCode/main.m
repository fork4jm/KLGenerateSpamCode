...
void modifyClassNamePrefix(NSMutableString *projectContent, NSString *sourceCodeDir, NSArray<NSString *> *ignoreDirNames, NSString *oldName, NSString *newName) {
    ...
    for (NSString *filePath in files) {
        ...
        printf(" \n convert file : %s.%s   \n",fileName.UTF8String, fileExtension.UTF8String);
        ...
        if ([fileExtension isEqualToString:@"h"]) {
            ...
            if ([files containsObject:mFileName]) {
                ...
                @autoreleasepool {
                    modifyFilesClassName([NSString stringWithFormat:@"%@/..",gSourceCodeDir], fileName, newClassName);
                }
            } else {
                continue;
            }
            ...
            @autoreleasepool {
                modifyFilesClassName([NSString stringWithFormat:@"%@/..",gSourceCodeDir], fileName.stringByDeletingPathExtension, newClassName);
            }
        } else {
            continue;
        }
        ...
        // 修改工程文件中的文件名
        NSString *regularExpressionh = [NSString stringWithFormat:@"\\b%@.h\\b", fileName];
        NSString *newClassNameh = [NSString stringWithFormat:@"%@.h",newClassName];
        
        NSString *regularExpressionm = [NSString stringWithFormat:@"\\b%@.m\\b", fileName];
        NSString *newClassNamem = [NSString stringWithFormat:@"%@.m",newClassName];
        
        NSString *regularExpressionxib = [NSString stringWithFormat:@"\\b%@.xib\\b", fileName];
        NSString *newClassNamexib = [NSString stringWithFormat:@"%@.xib",newClassName];
        
        regularReplacement(projectContent, regularExpressionh, newClassNameh);
        regularReplacement(projectContent, regularExpressionm, newClassNamem);
        regularReplacement(projectContent, regularExpressionxib, newClassNamexib);
